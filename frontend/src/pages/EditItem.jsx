...
import { Package, IndianRupee, Clock, FileText, Image, Percent, Calendar, ChevronDown, Tag } from 'lucide-react';
...
const EditItem = () => {
  ...
  useEffect(() => {
    const fetchItem = async () => {
      try {
        ...
        if (response.ok) {
          // Check permissions
          const isSeller = data.seller?._id === user?._id || data.seller === user?._id;
          ...
          if (data.auctionStartDate) {
            const dateStr = new Date(data.auctionStartDate).toISOString().slice(0, 16);
            setAuctionStartDate(dateStr);
          }
        ...
        toast.error('Server error fetching item');
        navigate('/');
      } finally {
  ...
  if (pageLoading) {
    return <div className="text-center mt-20 text-slate-400">Loading item data...</div>;
  }
    ...
    <div className="max-w-3xl w-full mx-auto px-6 pb-16 text-left">
      <div className="glass-panel p-8 md:p-10">
        ...
        <p className="text-slate-400 text-sm mb-8">
          Update the item's details. Note that changing the starting price may only apply if no bids have been placed yet.
        </p>
        ...
        <form onSubmit={handleSubmit} encType="multipart/form-data">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="flex flex-col gap-5">
              <div className="flex flex-col gap-2">
                <label className="text-sm font-semibold text-slate-400">Product Image (Leave blank to keep existing)</label>
                <div 
